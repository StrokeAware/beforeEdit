...
function BrainViewer({ selectedSlug, catalog }) {
  ...
  // Map colors by simple name match
  const colorForName = (name) => {
    const lower = (name || "").toLowerCase();
    const hit = catalog.find((c) => lower.includes(c.slug.replace(/-/g, " "))); // crude heuristic
  ...
  useEffect(() => {
    ...
    renderer.localClippingEnabled = true; // enable capability
    ...
    planeRef.current = plane; // not attached until user moves slider
  ...
  // Update clipping when user moves slider or changes orientation
  useEffect(() => {
    ...
    // Attach plane to renderer + materials
  ...
  const handleUpload = async (file) => {
    try {
      ...
      loader.load(
        ...
        (err) => {
          console.error(err);
          setLoadingError("Failed to load model. Ensure GLB/GLTF (with embedded or accessible buffers/textures). ");
        }
      );
    } catch (e) {
      console.error(e);
  ...
  // When a region is selected in the list, lightly flash-highlight matching meshes (by name contains slug words)
  useEffect(() => {
        ...
        {!modelLoaded && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center">
              <div className="text-slate-400 font-semibold">Upload a 3D Brain Model to Begin</div>
              <div className="text-xs text-slate-500">GLB or GLTF</div>
            </div>
          </div>
        )}
        {loadingError && (
