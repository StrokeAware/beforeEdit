...
const Brain = () => {
  ...
  // โหลดโมเดล (รองรับ Draco + Meshopt)
  const loadBrainModel = async (file) => {
    try {
      ...
      // DRACO decoder (CDN ทางการของ Google)
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          if (brainGroupRef.current) {
            brainGroupRef.current.traverse((child) => {
              if (child.isMesh) {
          ...
          model.traverse((child) => {
            if (child.isMesh) {
              ...
              // เริ่มต้น "ไม่ใส่ clippingPlanes" เพื่อให้เห็นโมเดลเต็ม ๆ
              ...
              const idx = meshesRef.current.length;
              const fallbackName = `Region ${idx}`;
              ...
              regions.push({ name: child.name || fallbackName, color });
            }
          });
      ...
      setLoadingError('Error loading model: ' + err.message);
    }
  };
  ...
  useEffect(() => {
    ...
    renderer.setSize(mountRef.current.clientWidth, Math.max(1, mountRef.current.clientHeight));
    renderer.localClippingEnabled = true; // เปิดความสามารถ แต่ยังไม่ใส่ plane
    ...
    // Clipping plane (ยังไม่ผูกกับ renderer/material จนกว่าจะขยับสไลด์)
    ...
    const onClick = (e) => {
      if (Math.abs(e.clientX - previousMouseRef.current.x) > 5) return;
      const rect = renderer.domElement.getBoundingClientRect();
  ...
  // เปิด/อัปเดต clipping เมื่อผู้ใช้ขยับสไลด์เดอร์หรือเปลี่ยน orientation
  useEffect(() => {
    if (!clippingPlaneRef.current || !rendererRef.current) return;

    const plane = clippingPlaneRef.current;
    const normalized = (slicePosition - 50) / 50; // -1..1
    const pos = normalized * 5;
    ...
    // ใส่ plane ให้ renderer และ material ของทุก mesh
    rendererRef.current.clippingPlanes = [plane];
    meshesRef.current.forEach((mesh) => {
      if (!mesh.material) return;
      if (Array.isArray(mesh.material)) {
        mesh.material.forEach((m) => (m.clippingPlanes = [plane]));
      } else {
  ...
  const filteredRegions = detectedRegions.filter((r) =>
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <div className="p-6 space-y-6">
          {!modelLoaded && (
            <div className="border-2 border-dashed border-blue-300 rounded-lg p-6 text-center bg-blue-50">
              ...
              <p className="text-sm text-gray-600 mt-3">Accepts GLB or GLTF files</p>
              {loadingError && <p className="text-red-600 text-sm mt-2">{loadingError}</p>}
            </div>
          )}
          ...
          {modelLoaded && (
            <>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-xs text-green-700 mt-1">{detectedRegions.length} regions detected</p>
                <button
                  onClick={() => {
                    setModelLoaded(false);
                    setDetectedRegions([]);
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Selected Region</label>
                <div
                  className={`p-4 rounded-lg border-2 transition-all ${
                    selectedPart ? 'bg-blue-50 border-blue-300' : 'bg-gray-50 border-gray-200'
                  }`}
                >
                  ...
                  {selectedPart && (
                    <button onClick={() => setSelectedPart(null)} className="text-xs text-blue-600 hover:text-blue-700 mt-2">
                      Clear selection
                    </button>
                  )}
                </div>
                  ...
                  {filteredRegions.map((region, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="text-sm font-medium text-gray-800 break-words">{region.name}</span>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-3">Slice Position</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-3">Slice Orientation</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-3">Camera Views</label>
                <div className="grid grid-cols-3 gap-2">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Show Full Opacity</span>
                </label>
              </div>
      ...
      <div className="flex-1 relative">
        {/* บังคับให้มีความสูงอย่างน้อยกัน canvas สูง 0 */}
        <div ref={mountRef} className="w-full h-full" style={{ minHeight: '70vh' }} />
