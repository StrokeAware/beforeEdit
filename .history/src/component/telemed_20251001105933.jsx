...
const StrokeFormOverlay = () => {
  ...
  const riskInfo = useMemo(() => {
    ...
    if (percent >= 80) {
      ...
      label = t("risk3");
      color = "#fb8c00"; // เหลืองมาตรฐาน
    } else if (percent >= 20) {
      label = t("risk4");
      color = "#fdd835"; // เหลืองอ่อน
    } else {
      label = t("risk5");
      color = "#2e7d32";
    }
  ...
  return (
    <div style={{ padding: '20px', fontFamily: "Prompt" }} ref={formRef}>
      ...
      >
        คุณ {patientName ?? '-'}<LanguageSwitch />
        </div>
      ...
      {!loading && patientData && (
        <div className='Box-risk'>
          ...
          >
            {riskInfo.percent >= 60 ? "ปรึกษาด่วน" : "ปรึกษาด่วน"}
          </button>
          <div className='risk'>
            ความเสี่ยง :
