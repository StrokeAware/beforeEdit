...
export default function GazeTest() {
  ...
  function handleCapture() {
    if (!iris) return setStatus(t("notfound"));
    ...
    <div className="container">
      ...
      {summary && (
        <div className="result">
          <div className="result2"> Center: {summary.center.toFixed(2)} px</div>
          <div className="result2"> {t("lefteye1")}&nbsp;&nbsp;: {summary.left.toFixed(2)} px</div>
          <div className="result2"> {t("righteye1")}&nbsp;&nbsp;: {summary.right.toFixed(2)} px</div>
          <div className="result3"> {summary.summary}</div>
          <Link to="/BEFAST_MAIN_FACE" className='Buttonresult'>{t("Next")}</Link>
        </div>
      )}

      <div style={{ maxWidth: 600, marginTop: 50 }}>
        <Line data={chartData} />
      </div>  
    </div>
  );
